...
- Filter-aware classification now owns cancellation of Git and its process
  group on Unix, including required clean-filter helpers. The name-only diff
  path no longer leaves a non-cancellable blocking task behind. Captured
  stdout/stderr are bounded and drained concurrently without detached readers.
  Cancellation signals the group directly; successful completion disarms
  cleanup, with no sleep or shell-out on the success path. An isolated Linux
  fixture verifies termination of a filter ignoring TERM. This fixes a
  demonstrated cancellation defect; whether it explains ai-auto-writer's
  live 30s timeouts remains unproven. Strict staging timing and deployment
